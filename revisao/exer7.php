...
<form action="exer7.php" method="post">
    <div class="row">
        <div class="col">
            <label for="prazo_finalizacao" class="form-label">Prazo para Finalização do Projeto (dias):</label>
            <input type="number" name="prazo_finalizacao" id="prazo_finalizacao" class="form-control" required />
        </div>
        <div class="col">
            <label for="atividades_total" class="form-label">Quantidade Total de Atividades:</label>
            <input type="number" name="atividades_total" id="atividades_total" class="form-control" required />
        </div>
        <div class="col">
            <label for="atividades_desenvolvidas" class="form-label">Quantidade de Atividades Desenvolvidas:</label>
            <input type="number" name="atividades_desenvolvidas" id="atividades_desenvolvidas" class="form-control" required />
        </div>
        <div class="col">
            <label for="produtividade_diaria" class="form-label">Produtividade Diária da Equipe (atividades/dia):</label>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $atividades_restantes = calcularAtividadesRestantes($atividades_total, $atividades_desenvolvidas);
    $dias_necessarios = calcularDiasNecessarios($atividades_restantes, $produtividade_diaria);

    exibirResultado($dias_necessarios, $prazo_finalizacao, $atividades_restantes);
}

function calcularAtividadesRestantes($total, $desenvolvidas) {
    return $total - $desenvolvidas;
}

function calcularDiasNecessarios($restantes, $produtividade) {
    return ceil($restantes / $produtividade);
}

function exibirResultado($dias_necessarios, $prazo_finalizacao, $atividades_restantes) {
